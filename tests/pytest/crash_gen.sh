...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$(pwd)/../../debug/build/lib

# Now we are all let, and let's see if we can find a crash.
ulimit -c unlimited

ps -ef | grep -w taosd | grep -v grep | awk '{print $2}' && pkill taosd
../../debug/build/bin/taosd -c ../../debug/test/cfg &
sleep 5
./crash_gen.py
