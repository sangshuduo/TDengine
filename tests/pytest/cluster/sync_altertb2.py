...
class TDTestCase:
    ...
    def alterTable(self):
        ...
        cursor.execute('insert into tb1 values(%ld, %d, %f)' %
                       (self.startTime + self.rowsPerTable + 10, 1, 1.2))
        cursor.execute('insert into tb1 values(%ld, %d, %f)' %
                       (self.startTime + self.rowsPerTable + 20, 2, 1.4))
        cursor.execute('insert into tb1 values(%ld, %d, %f)' %
                       (self.startTime + self.rowsPerTable + 30, 3, 1.6))
    ...
    def run(self):
        ...
        thread = threading.Thread(
            target=self.alterTable,
            name="alter some tables in syncing")
        thread.start()
        threads.append(thread)
        for t in range(2):
            threads[t].join()
